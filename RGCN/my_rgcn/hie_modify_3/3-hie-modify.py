...
class HierarchicalRGCN(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, num_rels):
        ...
        self.fc_l2 = nn.Linear(h_dim, 3)   # attack / deception
        # self.fc_l3 = nn.Linear(h_dim, OUT_DIM)   # 子类
        self.fc_attack = nn.Linear(h_dim, 5)  # 4 known attack + attack_other
...
def build_global_l3_probs(preds):
    ...
    p3_prob = torch.zeros(batch_size, OUT_DIM, device=p2.device)

    # benign -> 全局 L3 的 0 类
    p3_prob[:, 0] = p2_prob[:, 0]

    # fraud -> 全局 L3 的 [1,2,3]
    p3_prob[:, 1:4] = p2_prob[:, 2].unsqueeze(1) * p_fraud_prob

    # attack -> 全局 L3 的 [4,5,6,7,8]
    p3_prob[:, 4:9] = p2_prob[:, 1].unsqueeze(1) * p_attack_prob
...
def compute_loss(preds, labels,
    ...
    benign_l3_prob = p3_prob[:, 0]
    malicious_l3_prob = p3_prob[:, 1:].sum(dim=1)
    ...
    attack_l3_prob = p3_prob[:, 4:9].sum(dim=1)
    fraud_l3_prob = p3_prob[:, 1:4].sum(dim=1)
...
def test_model(model, test_dataloader, test_subset):
    ...
    return test_accuracy, precision, recall, f1, cm, test_time, attack_wrong_records
...
test_accuracy, precision, recall, f1, cm, test_time, attack_wrong_records = test_model(
    t_model,
    test_dataloader,
    test_subset
)
...
#     plt.legend()
#     plt.savefig(f"RGCN/losses/hie/hie_others_loss_curve_{len(dataset)}_{time.time()}.png")
...
#     plt.legend()
#     plt.savefig(f"RGCN/accs/hie/hie_others_accuracy_curve_{len(dataset)}_{time.time()}.png")
...
def plot_loss_acc_curves(train_losses, val_losses, train_acc, val_acc, config, dataset_size):
    ...
    exp_dir = f"RGCN/experiments/3/hie_exp_{dataset_size}_{timestamp}"
