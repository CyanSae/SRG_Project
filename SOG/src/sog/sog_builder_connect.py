...
class SOGBuilder:
    ...
    def build(self):
        ...
        self.connect_isolated_subgraphs()
    ...
    def build_regions(self):
        ...
        for n in self.blocks:
            ...
            self.regions[n.ident()] = control_node  
    ...
    def find_previous_pc(self, sorted_pc_values, target_pc):
        previous_pc = None
        for pc in sorted_pc_values:
            if pc >= target_pc:
                break
            previous_pc = pc
        return previous_pc

    def connect_isolated_subgraphs(self):
        components = list(nx.weakly_connected_components(self.sog))
        # print(f"oc:{len(components)}")
        # print(f"oe:{len(self.sog.edges())}")
        # print(len(self.sog.nodes()))
        if len(components) <= 1:
            return

        pc_values = [int(str(node).split(':')[0], 16) for node in self.sog.nodes()]
        sorted_pc_values = sorted(pc_values)
        
        for comp in components:
            min_pc = None
            for node in comp:
                pc_part = str(node).split(":")[0]
                try:
                    pc = int(pc_part, 16)
                    if min_pc is None or pc < min_pc:
                        min_pc = pc
                except ValueError:
                    continue
            
            previous_pc = self.find_previous_pc(sorted_pc_values, min_pc)
        
            if previous_pc is not None:
                pred = self.cfg.get_ops_by_pc(int(previous_pc))
                min_node = self.cfg.get_ops_by_pc(int(min_pc))
                mn = sognode.SOGNode.from_tac_op(min_node[0])
                pn = sognode.SOGNode.from_tac_op(pred[0])
                self.sog.add_edge(mn.op_name, pn.op_name, type='connection', color='yellow')
                # print(f"The previous PC value before {hex(min_pc)} is {hex(previous_pc)}.")
            else:
                continue
                # print(f"There is no PC value before {hex(min_pc)} in the list.")

        new_components = list(nx.weakly_connected_components(self.sog))
        print(f"nc:{len(new_components)}")
        # print(len(self.sog.edges()))
        # print(len(self.sog.nodes()))
...
if __name__ == "__main__":
    ...
    # nx.draw(sog, pos=nx.circular_layout(sog), with_labels=True, edge_color=['red', 'blue', 'green', 'yellow'])
    # # 'blue', 'green', pos=nx.circular_layout(sog), pos=nx.random_layout(sog), 
    # plt.show()
